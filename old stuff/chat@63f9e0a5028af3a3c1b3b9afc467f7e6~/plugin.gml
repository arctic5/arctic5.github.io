...
ds_list_add(global.chatLog,"OLorgan3's chat plugin v4 - press T to show/hide,");
...
object_event_add(Player,ev_step,ev_step_begin,'
    ...
    if !variable_local_exists("hasChat") {
        if instance_exists("global.chatWindow") && global.isHost {
...
object_event_add(global.chatWindow,ev_other,ev_user0,'
    while(PluginPacketGetBuffer(global.chatPacketID) != -1) {
        ...
        switch(read_ubyte(chatReceiveBuffer)) {
            case 0: //hello
                if ds_list_find_index(chatBanlist,socket_remote_ip(_player.socket)) == -1 {  //ignore muted players
    ...
    if string_copy(_message,1,1) != '/' {
        ...
        if !found {
            if ds_map_exists(global.commandMap,string_lower(ds_list_find_value(arguments,0))) {
                if ds_list_find_index(global.blockedCommands,string_lower(ds_list_find_value(arguments,0))) >= 0 && _player!= global.myself && string_lower(ds_list_find_value(arguments,0)) != 'help' {
                    _team = 0;
                    playerId = 255;
